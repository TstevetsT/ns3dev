...
/////////////Keeps a buffer of recent sent packets for loop control
/*
...
/************************************************START DGGF************************************
      ...
      double c = 299792458;
//      double DGGFTxMaxRgTime = (double)(DGGFTxMaxRg)/c;
      double DGGFTxMaxRgTime = (double)(DGGFTxMaxRg+1)/c;  //Adding 1 to max range may eliminate zero times
      double DGGFdelayTimer = DGGFTxMaxRgTime;
      double dpd = 0.0;
      double tpd = 0.0;
      double drp = 0.0;
      double trp = 0.0;
...
// This section of code will test if the recieving node(r) is closer to destination(d) than previous node(p)
// If ignorepacketsrange is 0, than packet will be dropped if r is further from d than p
// If ignorepacketsrange is positive x, r nodes that are x not at least x closer to d than p will be dropped
// If ignorepacketsrange is negative x, r nodes that are x further from d than p will still be considered.
// This results in a tradeoff of dropped packets (less if x is bigger) vs network congestion (less if x is smaller)
      double ignorepacketsrange=double(fwdArch);       
      if (drd+ignorepacketsrange > dpd)
      {
        NS_LOG_INFO (" Pnode is closer to Dnode than Rnode " << m_node->GetId () << p);
        return RX_ENDPOINT_UNREACH;  // Drop packet if distance from received packet to dest + ignorepacketsrange 
									//   is greater that distance from previous packet to dest
      }
 ...
 //     std::cout << "SimTime: " << Simulator::Now () << "\tTimer: " << DGGFdelayTimer
 //               << "\tdrd:" << drd << "\tdpd:" << dpd << "\n";
      if (DGGFdelayTimer < 0)
      {
        std::cout << "*[error]*****************************************************************************\n" 
                  << " [error]               Delay Timer was Negative, using abs!                 \n" 
                  << " [error] " << DGGFTxMaxRgTime << "*2+" << trd << "-" << trp << "-" << tpd << "=" << DGGFdelayTimer << "\n"
                  << " [error]             DGGFTxMaxRgTime*2+trd-trp-tpd=DGGFdelayTimer                     \n"
                  << "*[error]*****************************************************************************\n";
        // DGGFdelayTimer=abs(DGGFdelayTimer);
        return RX_ENDPOINT_UNREACH;  // Drop packet if delay time is negative value
      }
      // Multiply by a scaling factor
//      std::cout << DGGFTxMaxRgTime << "*2+" << trd << "-" << trp << "-" << tpd << "=" << DGGFdelayTimer << " 2*maxrg/c+trd-trp-tpd=timer \n";
      DGGFdelayTimer = DGGFdelayTimer*double(sFactor);
//       search output file for string "terminate called without an active exception"
//  Troubleshoot case when delay timer is negative 
